...
import { AUTH_PROXY_URL } from '../constants';
...
export async function signInWithGoogle(): Promise<GoogleAuthResult | null> {
  ...
  // Prefer explicit proxy URL from env if provided and valid
  const explicitProxy = (AUTH_PROXY_URL || '').match(/auth\.expo\.(dev|io)/) ? AUTH_PROXY_URL : '';
  // Candidate generated by AuthSession (may fallback to custom scheme in Dev Client)
  const generatedProxy = makeRedirectUri({ useProxy: true, preferLocalhost: false, isTripleSlashed: true });
  ...
  if (usingIOSClient) {
    ...
    if (auth.type !== 'success') {
      ...
      if (webClientId) {
        try {
          const proxyRedirect = makeRedirectUri({ useProxy: true, preferLocalhost: false, isTripleSlashed: true });
          ...
          await fallback.makeAuthUrlAsync(discovery, { useProxy: true });
          const res = await fallback.promptAsync(discovery, { useProxy: true });
          if (res.type === 'success') {
    ...
    if (Platform.OS === 'ios' && iosClientId) {
      ...
      try {
        await nativeReq.makeAuthUrlAsync(discovery, { useProxy: false });
        const auth = await nativeReq.promptAsync(discovery, { useProxy: false });
